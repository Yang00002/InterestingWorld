...
public class RepeatSlashingAbility extends CommonAbility
{
	...
	RepeatSlashingAbility(IntrusiveTag<AbilityHeart> intrusiveTag)
	{
		super(intrusiveTag);
	}
	...
	public void postDamageEntity(ItemStack stack, LivingEntity target, LivingEntity attacker)
	{
		if (attacker instanceof ServerPlayerEntity player)
		{
			...
			if (manager.isAbilityOn() && manager.sweeping && !manager.isInCooldown(this) &&
				(step < 3 || manager.extractAtomicEnergy(stack, EnergyConsume)))
			{
				int damage;
				int range;
				float angle;
