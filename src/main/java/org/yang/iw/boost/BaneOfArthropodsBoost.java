...
public class BaneOfArthropodsBoost extends AbstractBoost
{
	BaneOfArthropodsBoost(Identifier identifier)
	{
		super(identifier);
	}
	...
	public BoostFunctionMap getFunctions(int level)
	{
		...
		}).add(AttributeModifierSlot.MAINHAND, new TargetDamagedFunction()
		{
			...
			{
				if (target.getType().isIn(EntityTypeTags.SENSITIVE_TO_BANE_OF_ARTHROPODS))
					target.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, 20 + 10 * level, 3),
