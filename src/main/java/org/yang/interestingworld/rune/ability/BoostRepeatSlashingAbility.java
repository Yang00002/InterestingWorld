...
import static org.yang.interestingworld.IWUtil.TextStyle.DARK_RED_RGB;
import static org.yang.interestingworld.IWUtil.TextStyle.GRAY_RGB;

public class BoostRepeatSlashingAbility extends IWRuneAbility
{
	...
	public void postDamageEntity(ItemStack stack, LivingEntity target, LivingEntity attacker)
	{
		if (attacker instanceof ServerPlayerEntity player)
		{
			...
			if (manager.charged)
			{
				...
				IWUtil.EnergyTool.sweepEntity(attacker, angle, range, target, (attacker1, entity, distance) -> {
					if (step > 2) entity.takeKnockback(ThirdAttackKnockback, knox, knoz);
					entity.damage(damageSource, damage);
					double x = entity.getX();
					double y = entity.getBodyY(0.5);
					double z = entity.getZ();
					IWUtil.Network.spawnParticlesAtPos(world, ParticleTypes.SWEEP_ATTACK, x, y, z, pc, 0.5, 0.5, 0.5);
				});
			}
		}
	}
	...
	{
		return data.charged ? DARK_RED_RGB : GRAY_RGB;
	}
